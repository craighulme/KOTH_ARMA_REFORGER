...
class KOTH_SpectatorCamera : CameraBase
{
	...
	protected static IEntity cameraEnt;
	...
	static KOTH_SpectatorCamera GetInstance()
	{
		...
		cameraEnt = GetGame().SpawnEntityPrefabLocal(res);
	...
	static void Destroy()
	{
		...
		RplComponent rplComp = RplComponent.Cast(s_Instance.FindComponent(RplComponent));
		rplComp.DeleteRplEntity(cameraEnt, false);
		//SCR_EntityHelper.DeleteEntityAndChildren(cameraEnt);
		s_Instance = null;
	}
