class KOTH_ShopWeaponRearmUI : ChimeraMenuBase
{
	...
	private void ComputeRearmPrice()
	{
		IEntity playerEnt = GetGame().GetPlayerController().GetControlledEntity();
		int totalPrice = KOTH_Helper.ComputeRearmPrice(playerEnt, GetGame().GetPlayerController());
	...
	{
		m_playerShopComp.DoAskRpc_RearmLoadout(m_playerId);
